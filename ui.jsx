  ...
  if (typeof value === "object" && value.cross) {
    ...
    return (
      <span className={"delta " + (up ? "up" : "down")}>
        {up ? <IconArrowUp /> : <IconArrowDown />} {t(up ? 'turn_positive' : 'turn_negative')}
      </span>
    );
  }
...
function RatingPill({ rating, label, size }) {
  const map = { 
    bull: { t: t('rating_bull'), ic: <IconArrowUp /> }, 
    bear: { t: t('rating_bear'), ic: <IconArrowDown /> }, 
    neutral: { t: t('rating_neutral'), ic: <IconArrowRight /> } 
  };
...
function DiscBadge({ grade }) {
  return <span className={"disc-badge disc-" + grade} title={t('disclosure_grade_title') + " " + grade}>{grade}</span>;
}
...
function Missing({ text }) {
  return <span className="missing"><IconInfo style={{ width: 12, height: 12 }} />{text || t('undisclosed')}</span>;
}
